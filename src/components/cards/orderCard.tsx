...
export default function AgendaCodeMobile({ ...props }) {
  ...
  useEffect(() => {
    // If showPdf is true, set a timeout to automatically close the InvoicePDF component after 5 seconds
    if (showPdf) {
      const timeoutId = setTimeout(() => {
        setShowPdf(false);
      }, 500); // 5000 milliseconds (5 seconds)

      // Clear the timeout if the component is unmounted or if showPdf becomes false before the timeout triggers
