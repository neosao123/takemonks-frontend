...
export default function ShopProductCard({ ...props }) {
  ...
  const isMaxQuantity =
    !isLoading &&
    checkout.cart
      .filter((item: any) => item.id === product?._id)
      .map((item: any) => item.quantity)[0] >= product?.available;
  // console.log(checkout.cart, isMaxQuantity, "inside in the product cart");
  return (
    <Card
      sx={{
        display: "block",
        ...
        <Stack flexDirection="row" justifyContent="space-between">
          {loading ? (
            ...
            <Button
              ...
              fullWidth
              disabled={loading || isMaxQuantity || product?.available < 1}
              onClick={() => onAddCart()}
            >
