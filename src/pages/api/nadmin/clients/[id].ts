...
export default async function handler(
  ...
  switch (method) {
    case "GET" /* Get a model by its ID */:
      try {
        const user = await clients.findOne({ _id: id });
    ...
    case "PUT" /* Edit a model by its ID */:
      try {
        ...
        const phoneExist = await clients.findOne({
          _id: { $ne: id },
        ...
        if (!user) {
          return res
            .status(200)
            .json({ success: false, message: "No client User found!" });
        }
    ...
    case "DELETE":
      try {
        const findUser = await clients.findByIdAndUpdate(
          id,
          { isUserDeleted: true },
          { new: true }
        );
